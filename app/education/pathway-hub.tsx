...
export async function PathwayHub({ pathway }: { pathway: PathwaySlug }) {
  const config = getPathwayConfig(pathway)
  const norm = normalizePathway(pathway)
  if (norm === '') return notFound()

  const [herbs, compounds] = await Promise.all([getHerbs(), getCompounds()])

  const relatedHerbs = (getRelatedPathwayRecords(herbs, norm) as RuntimeRecord[])
    .filter((record: RuntimeRecord) => getRuntimeVisibility(record).canRender)
    .slice(0, 9)

  const relatedCompounds = (getRelatedPathwayRecords(compounds, norm) as RuntimeRecord[])
    .filter((record: RuntimeRecord) => getRuntimeVisibility(record).canRender)
    .slice(0, 9)
      ...
      {relatedHerbs.length > 0 ? (
        <section className="space-y-5">
          <div className="space-y-2">
            ...
            <h2 className="text-3xl font-semibold tracking-tight text-ink">Herbs related to {getPathwayLabel(norm)}</h2>
          </div>
      ...
      {relatedCompounds.length > 0 ? (
        <section className="space-y-5">
          <div className="space-y-2">
            ...
            <h2 className="text-3xl font-semibold tracking-tight text-ink">Compounds related to {getPathwayLabel(norm)}</h2>
          </div>
