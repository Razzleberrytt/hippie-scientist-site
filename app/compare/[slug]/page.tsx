        ...
        {cards.map(card => (
          <article key={card.key} className='rounded-3xl border border-white/10 bg-white/[0.04] p-5'>
            ...
            <div className='mt-4 flex flex-wrap gap-2'>
              ...
              {card.record ? <Link href={profileHref(card.record)} className='rounded-2xl border border-white/10 px-4 py-2 text-sm font-bold text-white/75 transition hover:bg-white/5 hover:text-white'>Open profile</Link> : null}
            </div>
          </article>
        ))}
      </section>
