...
const lanes = [
  {
    ...
    status: 'Continuous',
    body: 'Verify high-traffic claims, improve source classification, refresh citations, and strengthen outcome-specific evidence summaries before expanding weak pages.',
  },
  {
    title: 'Safety and interactions',
    status: 'Continuous',
    body: 'Improve interaction provenance, uncertainty labels, safety completeness, and educational pairwise resources without treating missing data as proof of safety.',
  },
  {
    title: 'Research tools',
    status: 'Active',
    body: 'Expand the Evidence Database, Safety Checker, Botanical Activity Atlas, Citation Explorer, evidence-change tracking, and downloadable original-data resources.',
  },
  {
    title: 'High-demand content',
    status: 'Demand-ranked',
    body: 'Upgrade pages and comparisons that show measurable search demand, reader demand, or evidence gaps rather than expanding the database evenly by alphabetical order.',
  },
] as const
    ...
    <main className='container-page space-y-8 py-10'>
      <section className='hero-shell rounded-[2rem] border border-brand-900/10 p-6 shadow-card sm:p-8 lg:p-10'>
        ...
        <p className='mt-5 max-w-3xl text-lg leading-8 text-muted'>
          Work is prioritized by evidence and safety importance, measurable search demand, reader demand, expected impact, confidence, and implementation effort. Requests can influence the queue, but they do not automatically change scientific conclusions or evidence grades.
        </p>
      ...
      <section aria-labelledby='roadmap-lanes' className='space-y-4'>
        <div>
          <p className='eyebrow-label'>Current lanes</p>
          <h2 id='roadmap-lanes' className='mt-2 text-2xl font-semibold tracking-tight text-ink'>The work streams that stay in view</h2>
        </div>
        <div className='grid gap-4 md:grid-cols-2'>
          {lanes.map((lane) => (
            <article key={lane.title} className='card-premium p-6'>
              <div className='flex items-start justify-between gap-4'>
                <h3 className='text-lg font-bold text-ink'>{lane.title}</h3>
                <span className='rounded-full border border-brand-900/10 bg-brand-50 px-2.5 py-1 text-xs font-bold text-brand-800'>{lane.status}</span>
              </div>
      ...
      <section className='rounded-2xl border border-amber-900/15 bg-amber-50/60 p-5 text-sm leading-6 text-amber-950'>
        <strong>Privacy:</strong> public requests are for research topics and public sources only. Do not submit symptoms, diagnoses, medication lists, medical records, or other sensitive personal health information.
      </section>
