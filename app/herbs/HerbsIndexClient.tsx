...
function StatCard({ value, label }: { value: number; label: string }) {
  return (
    <div className="rounded-[1rem] border border-brand-900/10 bg-white/75 p-2.5 shadow-sm backdrop-blur sm:rounded-[1.1rem] sm:p-4">
      <p className="text-2xl font-semibold tracking-tight text-ink sm:text-3xl">{value}</p>
      <p className="mt-1 text-xs font-bold uppercase tracking-[0.1em] text-[#5f6f66]">{label}</p>
    </div>
  )
}
...
function EmptyFilteredState({ query, context }: { query: string; context: string }) {
  ...
  const currentFocus = [query ? `“${query}”` : '', activeContext || ''].filter(Boolean).join(' + ')

  return (
    <DecisionEmptyState
      eyebrow="No matching profiles"
      title="No herbs matched this focus."
      ...
      currentScan={currentFocus || undefined}
      actions={[
  ...
  return (
    <div className="min-h-screen px-3 pb-24 pt-4 text-ink sm:px-4 sm:py-8">
      <div className="mx-auto max-w-7xl space-y-6 sm:space-y-10 lg:space-y-12">
        <section className="hero-shell relative overflow-hidden rounded-[1.45rem] border border-white/50 px-4 py-6 shadow-sm sm:rounded-[2.2rem] sm:px-8 sm:py-10 lg:px-12">
          ...
          <div className="relative grid gap-5 lg:grid-cols-[1.05fr_.95fr] lg:items-end">
            <div className="max-w-3xl space-y-3.5 sm:space-y-4">
              ...
              <p className="max-w-2xl text-base leading-7 text-[#46574d] sm:text-lg sm:leading-8">
                Start with practical context, then compare evidence, safety, and timing before opening a full herb profile.
              </p>
            </div>

            <div className="rounded-[1.2rem] border border-brand-900/10 bg-white/75 p-3 shadow-sm backdrop-blur sm:rounded-[1.35rem] sm:p-4">
              <p className="text-xs font-bold uppercase tracking-[0.12em] text-brand-800">Library signal</p>
              <div className="mt-3 grid grid-cols-3 gap-2">
                <StatCard value={totalProfiles} label="Profiles" />
                <StatCard value={evidenceForward} label="Evidence" />
                <StatCard value={safetyMapped || readyProfiles} label="Safety" />
              </div>
            </div>
          </div>
        </section>

        <section className="rounded-[1.3rem] border border-brand-900/10 bg-white/85 p-4 shadow-[var(--shadow-card-calm)] sm:rounded-[1.5rem] sm:p-6" aria-labelledby="herb-search-heading">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <Link href="/goals" className="inline-flex min-h-11 w-fit items-center rounded-full px-1 text-sm font-bold text-brand-800 transition hover:text-brand-900">Browse all goals →</Link>
          </div>

          <form action="/herbs" className="mt-4 grid gap-3 sm:mt-5 sm:grid-cols-[1fr_auto]">
            ...
            <input
              ...
              className="min-h-13 w-full rounded-full border border-brand-900/10 bg-white px-4 text-base text-ink shadow-sm placeholder:text-[#7b8a81] sm:min-h-12"
            />
        ...
        <section className="rounded-[1.3rem] border border-brand-900/10 bg-white/70 p-4 shadow-sm sm:rounded-[1.5rem] sm:p-5">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {browsePaths.map(path => (
              <Link
                key={path.label}
                href={path.href}
                className="group min-h-24 rounded-[1rem] border border-brand-900/10 bg-white/80 p-4 shadow-sm transition active:scale-[0.995] hover:-translate-y-0.5 hover:border-brand-700/20 hover:bg-white sm:rounded-[1.1rem]"
              >
              ...
              <section className="space-y-4 sm:space-y-5">
                <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                  <div className="max-w-2xl space-y-2">
                    ...
                    <h2 className="compact-heading">Strong starting profiles for learning the card pattern.</h2>
                  </div>
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                  <div className="max-w-2xl space-y-2">
                    ...
                    <h2 className="compact-heading">
                      {hasActiveFilters ? 'Profiles matching your focus.' : 'Browse every published herb profile.'}
                    </h2>
                  </div>
