...
export async function generateStaticParams(){ const herbs=(await getHerbSummaryIndex()).filter((h: Record<string, unknown>)=>getRuntimeVisibility(h).canRender); const total=Math.max(1,Math.ceil(herbs.length/HERBS_PAGE_SIZE)); return Array.from({length:Math.max(total-1,0)},(_,i)=>({page:String(i+2)})) }
