  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-24 pt-8">
      ...
      <header className="mb-10">
        ...
        <p className="mt-4 text-lg text-muted leading-relaxed">And why most stack guides won&apos;t tell you.</p>
      </header>

      <section className="mb-10">
        <p className="text-sm leading-7 text-muted mb-4">Serotonin syndrome doesn&apos;t announce itself. It builds quietly across a stack — an SSRI prescription, a mood-support herb, a &ldquo;natural&rdquo; sleep aid — until the combination tips into agitation, rapid heart rate, and in severe cases, something that lands you in an emergency room. Most supplement content never mentions this risk, because most supplement content is written one ingredient at a time. Nobody&apos;s cross-referencing the whole catalog.</p>
        <p className="text-sm leading-7 text-muted">We did. Across our full reference set of 873 herbs and compounds, we built a deterministic interaction layer that flags every entity carrying a serotonergic contraindication and generates a caution edge for every pair that shares it. The serotonergic cluster is one of five mechanism groups in that layer — and at 18 entities and 153 flagged pairings, it&apos;s the smallest and most acute. This is that list.</p>
      </section>
      ...
      <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 mb-10">
        ...
        <p className="text-sm leading-7 text-amber-900">This is the distinction most lists skip, and it matters. Sixteen of these 18 entities are flagged for genuine serotonergic mechanisms — direct precursors (5-HTP, tryptophan), reuptake-adjacent botanicals (kanna, rhodiola, saffron), or MAOI-active alkaloids (harmaline, harmine). The risk there is classic serotonin syndrome.</p>
        <p className="mt-3 text-sm leading-7 text-amber-900"><strong>Mucuna and Yohimbe are different.</strong> Both surfaced in this cluster because their monographs explicitly caution against MAOIs — but the underlying danger is a <strong>hypertensive crisis</strong>, not serotonin syndrome. Mucuna is a levodopa source; combined with an MAOI, the concern is a dopaminergic/pressor reaction. Yohimbe is sympathomimetic; stacked with an MAOI or stimulant, the same pressor mechanism applies. Different physiology, same instruction: don&apos;t combine with MAOIs. We&apos;re keeping them in the list because the caution is real and the keyword match is accurate — but if you&apos;re trying to understand serotonin syndrome specifically, those two are the exception, not the rule.</p>
      </section>

      <section className="mb-10">
        <h2 className="text-2xl font-bold text-ink mb-4">Where this actually shows up in real stacks</h2>
        <p className="text-sm leading-7 text-muted mb-4">These pairings aren&apos;t hypothetical. A few combinations we see constantly in mood, sleep, and libido stacks — each one a flagged pair in our data:</p>
        <div className="space-y-3">{STACKS.map((s) => <div key={s.combo} className="rounded-xl border border-brand-900/10 bg-white p-4"><p className="text-sm font-semibold text-ink">{s.combo}</p><p className="text-xs text-muted mt-1 leading-5">{s.note}</p></div>)}</div>
        <p className="mt-4 text-sm leading-7 text-muted">None of this means any single ingredient is dangerous on its own at typical use. The risk is additive — it&apos;s the stack, or the stack on top of a prescription, that matters.</p>
      </section>

      <section className="mb-10">
        <h2 className="text-2xl font-bold text-ink mb-4">Recognizing serotonin syndrome</h2>
        <p className="text-sm leading-7 text-muted mb-4">This is general safety information, not a diagnosis tool — if you suspect you or someone else is experiencing this, treat it as a medical emergency. Symptoms typically appear within hours of a new combination or dose increase and can include:</p>
        <ul className="list-disc pl-5 space-y-1.5 text-sm text-muted mb-4">{SYMPTOMS.map((s) => <li key={s}>{s}</li>)}</ul>
        <div className="rounded-xl border border-red-200 bg-red-50 p-4"><p className="text-sm font-bold text-red-900">If severe symptoms appear, this warrants emergency medical attention — call emergency services rather than waiting it out.</p></div>
      </section>

      <section className="rounded-2xl border border-brand-700/20 bg-brand-50/60 p-6 mb-10">
        <h2 className="text-xl font-bold text-ink mb-2">Use the interactive tools</h2>
        <p className="text-sm leading-7 text-muted mb-4">Every herb and compound page includes a live &ldquo;Caution When Combined With&rdquo; panel. You can also compare botanicals carrying structured serotonergic safety signals in the activity atlas, sorted by strongest evidence.</p>
        <div className="flex flex-wrap gap-3">
          <Link href={SEROTONERGIC_ATLAS_HREF} className="inline-flex min-h-[44px] items-center rounded-full bg-brand-700 px-5 text-sm font-semibold text-white transition hover:bg-brand-800">Compare serotonergic-risk botanicals →</Link>
          <Link href="/herbs/kanna/" className="inline-flex min-h-[44px] items-center rounded-full border border-brand-900/10 px-5 text-sm font-semibold text-brand-800 transition hover:bg-white">Start with Kanna →</Link>
        </div>
      </section>

      <section className="mb-10">
        <h2 className="text-2xl font-bold text-ink mb-4">References</h2>
        <div className="space-y-3">{REFERENCES.map((ref, i) => <div key={i} className="rounded-xl border border-brand-900/10 bg-white p-4"><p className="text-sm font-semibold text-ink">{ref.title}</p><p className="text-xs text-muted mt-1">{ref.authors}. <em>{ref.journal}</em> {ref.detail} ({ref.year}).{' '}<a href={`https://pubmed.ncbi.nlm.nih.gov/${ref.pmid}/`} target="_blank" rel="noopener noreferrer" className="text-brand-700 hover:underline">PMID: {ref.pmid}</a></p></div>)}</div>
      </section>

      <footer className="border-t border-brand-900/10 pt-6 text-xs text-muted leading-relaxed">This article reflects mechanistic cautions derived from documented contraindications across our reference catalog, not clinical trial data on combined use. It&apos;s intended to inform, not replace, a conversation with a clinician or pharmacist — especially if you&apos;re currently taking an SSRI, SNRI, or MAOI. The 18-entity serotonergic set and 153 pairwise edges were current as of 2026-06-30.</footer>
    </div>
  )
}
