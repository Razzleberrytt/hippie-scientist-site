...
export const metadata: Metadata = {
  ...
  description: 'A practical guide to herbs commonly discussed for cortisol, stress response, adaptogen support, and calm.',
...
export default async function CortisolPage() {
  ...
  const fallback = herbs.filter(herb => matches(herb) && !preferred.some(item => item.slug === herb.slug)).sort((a, b) => scoreFor(b) - scoreFor(a)).slice(0, Math.max(0, 6 - preferred.length))
  ...
  return (
    <main className='mx-auto max-w-5xl space-y-6 px-4 py-8 text-white sm:px-6 lg:px-8'>
      <section className='rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.18),transparent_35%),linear-gradient(135deg,rgba(255,255,255,0.07),rgba(255,255,255,0.025))] p-6 shadow-2xl shadow-black/25 sm:p-8'>
        <p className='text-xs font-bold uppercase tracking-[0.2em] text-emerald-100/70'>Hormone stress guide</p>
        <h1 className='mt-3 text-4xl font-black tracking-tight sm:text-6xl'>Best Herbs for Cortisol Support</h1>
        <p className='mt-4 max-w-3xl text-base leading-7 text-white/70'>A quick guide to herbs commonly discussed for stress response, cortisol context, and adaptogen-style support. Educational only, not medical advice.</p>
      </section>
      ...
      <section className='rounded-3xl border border-white/10 bg-white/[0.035] p-5 sm:p-6'>
        ...
        <p className='mt-3 text-sm leading-6 text-white/70'>Ashwagandha, rhodiola, and holy basil are commonly discussed when people search for herbs related to cortisol and stress resilience.</p>
      </section>
