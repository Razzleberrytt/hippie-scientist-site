          ...
          return (
            <article key={herb.slug} className='rounded-3xl border border-white/10 bg-white/[0.04] p-5'>
              ...
              <div className='mt-5 flex flex-wrap gap-2'>
                {links[0] ? (
                  <a href={links[0].url} target='_blank' rel='noopener noreferrer sponsored' className='rounded-2xl bg-emerald-300 px-4 py-2 text-sm font-bold text-slate-950 hover:bg-emerald-200'>
                    View {label} products →
                  </a>
                ) : null}
                <Link href={`/herbs/${herb.slug}/`} className='rounded-2xl border border-white/10 px-4 py-2 text-sm font-bold text-white/75 hover:bg-white/5'>
                  Learn more →
                </Link>
              </div>
            </article>
