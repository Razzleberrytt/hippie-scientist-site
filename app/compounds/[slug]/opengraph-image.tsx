...
import { getCompoundSummaryIndex } from '../../../src/lib/runtime-summary-indexes'
...
export async function generateStaticParams() {
  const compounds = await getCompoundSummaryIndex()
  return compounds
    .filter((c: any) => getRuntimeVisibility(c).canRender)
    .map((c: any) => ({ slug: String(c.slug) }))
}

export default async function OGImage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const compound = await getCompoundMetadataRecord(slug)
  const name = String(compound?.displayName || compound?.name || slug)
  const grade = String(compound?.evidence_grade || compound?.evidenceTier || 'Research')
    .split(/[–—]/)[0].trim()
  const summary = String(compound?.summary || '').slice(0, 120)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', width: '100%', borderRadius: '32px', border: '1px solid rgba(16,32,24,0.08)', background: 'rgba(255,255,255,0.6)', padding: '48px' }}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 18, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(16,32,24,0.5)', marginBottom: '16px' }}>
              The Hippie Scientist · Compound Profile
            </div>
