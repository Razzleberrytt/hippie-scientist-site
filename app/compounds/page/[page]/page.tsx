...
export async function generateStaticParams(){ const compounds=(await getCompounds()).filter((c:any)=>getRuntimeVisibility(c).canRender); const total=Math.max(1,Math.ceil(compounds.length/COMPOUNDS_PAGE_SIZE)); return Array.from({length:Math.max(total-1,0)},(_,i)=>({page:String(i+2)})) }
