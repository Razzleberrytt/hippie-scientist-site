...
import { DEFAULT_OG_LOCALE, GERMAN_OG_LOCALE } from '@/src/lib/international-seo'
...
export function generateStaticParams() {
  return getProfileTranslationSlugs('de', 'herb').map((slug) => ({ slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params
  const translation = getProfileTranslation('de', 'herb', slug)
  ...
  return {
    ...metadata,
    openGraph: metadata.openGraph ? { ...metadata.openGraph, locale: GERMAN_OG_LOCALE, alternateLocale: [DEFAULT_OG_LOCALE] } : metadata.openGraph,
  }
}

export default async function GermanHerbProfilePage({ params }: PageProps) {
  const { slug } = await params
  const translation = getProfileTranslation('de', 'herb', slug)
  if (!translation) notFound()
  const canonical = loadCanonicalLocalizedProfile('herb', slug)
  return <LocalizedResearchProfilePage canonical={canonical} translation={translation} ui={LOCALIZED_PROFILE_UI.de} lang='de' libraryHref='/de/kraeuter/' />
}
