...
export default function Home() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (search.trim()) {
      // Just route to herbs with query param for now
      ...
      <div className="flex flex-col gap-24 py-12 pb-24">
        ...
        <section className="flex flex-col items-center text-center max-w-3xl mx-auto space-y-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="text-5xl md:text-7xl font-outfit font-bold tracking-tight text-white mb-6">
          ...
          >
            ...
            <div className="relative flex items-center">
              ...
              <Input 
                ...
                className="w-full pl-12 pr-4 py-6 text-lg bg-black/40 border-white/10 rounded-2xl focus-visible:ring-primary focus-visible:border-primary backdrop-blur-sm"
              />
            </div>
          </motion.form>
          ...
          >
            <div className="flex flex-col items-center bg-white/5 border border-white/5 px-6 py-4 rounded-xl backdrop-blur-sm">
              <span className="text-3xl font-bold font-mono text-white mb-1">{isLoadingHerbs ? "..." : herbs?.length || 295}</span>
              <span className="text-xs text-muted-foreground uppercase tracking-widest">Herbs Profiled</span>
            </div>
            <div className="flex flex-col items-center bg-white/5 border border-white/5 px-6 py-4 rounded-xl backdrop-blur-sm">
              <span className="text-3xl font-bold font-mono text-white mb-1">{isLoadingCompounds ? "..." : compounds?.length || 617}</span>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 rounded-2xl border border-white/5 bg-black/20 backdrop-blur-sm space-y-4">
            <div className="w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center border border-blue-500/20">
              <Database className="w-6 h-6 text-blue-400" />
            </div>
            <h3 className="text-xl font-bold text-white">Mechanism First</h3>
          ...
          <div className="p-6 rounded-2xl border border-white/5 bg-black/20 backdrop-blur-sm space-y-4">
            ...
            <h3 className="text-xl font-bold text-white">Evidence Tiers</h3>
            <p className="text-muted-foreground text-sm leading-relaxed">
          ...
          <div className="p-6 rounded-2xl border border-white/5 bg-black/20 backdrop-blur-sm space-y-4">
            ...
            <h3 className="text-xl font-bold text-white">Harm Reduction</h3>
            <p className="text-muted-foreground text-sm leading-relaxed">
          ...
          <section>
            ...
            {isLoadingHerbs ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[...Array(4)].map((_, i) => <div key={i} className="h-48 rounded-xl bg-white/5 animate-pulse" />)}
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
