  ...
  return (
    <PageLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-4xl font-outfit font-bold tracking-tight text-foreground mb-4">Supplement Stacks</h1>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-8">
            {stacks?.map((stack) => (
              <div key={stack.slug} className="border border-border bg-card rounded-2xl overflow-hidden">
                <div className="p-6 border-b border-border bg-muted/30">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h2 className="text-2xl font-bold font-outfit text-foreground mb-2 flex items-center gap-2">
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {stack.stack.map((item, i) => (
                      <div key={i} className="bg-muted rounded-xl p-4 border border-border">
                        <h4 className="font-semibold text-foreground mb-1">{item.compound}</h4>
                        ...
                        {item.timing && (
                          <p className="text-xs text-muted-foreground mt-2">
                            <span className="font-semibold text-foreground/70">Timing: </span>
                            {item.timing}
                          </p>
                        )}
                      </div>
