...
text = replace_once(
    ...
    'apply: sheet-aware column error',
)
text = replace_once(
    text,
...
text = replace_once(
    ...
    'apply: external review validation',
)
text = replace_once(
    text,
...
text = replace_once(
    ...
    'apply: pass sheet to apply',
)
apply_path.write_text(text)

editor_path = Path('scripts/data/edit-entity-master-cell.mjs')
...
text = replace_once(
  ...
  return { sheetName: resolved.name, worksheetPath }
    ...
    'editor: sheet resolver',
)
text = replace_once(text, 'function getHeaderMap(rows) {', 'function getHeaderMap(rows, sheetName) {', 'editor: header signature')
text = replace_once(text, 'if (!headerRow) fail(`${ENTITY_SHEET_NAME} is missing header row 1`)', 'if (!headerRow) fail(`${sheetName} is missing header row 1`)', 'editor: header error')
text = replace_once(text, 'function findTargetRow(rows, slugColumn, wantedSlug) {', 'function findTargetRow(rows, slugColumn, wantedSlug, sheetName) {', 'editor: row finder signature')
text = replace_once(text, 'if (matches.length === 0) fail(`Slug not found in ${ENTITY_SHEET_NAME}: ${wantedSlug}`)', 'if (matches.length === 0) fail(`Slug not found in ${sheetName}: ${wantedSlug}`)', 'editor: slug missing')
text = replace_once(text, 'if (matches.length > 1) fail(`Duplicate slug in ${ENTITY_SHEET_NAME}: ${wantedSlug} appears on rows ${matches.map((row) => row.rowNumber).join(\', \')}`)', 'if (matches.length > 1) fail(`Duplicate slug in ${sheetName}: ${wantedSlug} appears on rows ${matches.map((row) => row.rowNumber).join(\', \')}`)', 'editor: duplicate slug')
text = replace_once(text, 'function editCell({ workbookPath, slug, column, value, dryRun }) {', 'function editCell({ workbookPath, sheet, slug, column, value, dryRun }) {', 'editor: edit signature')
text = replace_once(text, 'const worksheetPath = locateEntityMasterWorksheet(entryMap)', 'const { sheetName, worksheetPath } = locateEntityMasterWorksheet(entryMap, sheet)', 'editor: resolved worksheet')
text = replace_once(text, 'const headers = getHeaderMap(rows)', 'const headers = getHeaderMap(rows, sheetName)', 'editor: header call')
text = replace_once(text, 'if (!slugColumn) fail(`${ENTITY_SHEET_NAME} is missing required slug column`)', 'if (!slugColumn) fail(`${sheetName} is missing required slug column`)', 'editor: slug column error')
text = replace_once(text, 'if (!targetColumn) fail(`${ENTITY_SHEET_NAME} is missing target column: ${normalizedColumn}`)', 'if (!targetColumn) fail(`${sheetName} is missing target column: ${normalizedColumn}`)', 'editor: target column error')
text = replace_once(text, 'const targetRow = findTargetRow(rows, slugColumn, wantedSlug)', 'const targetRow = findTargetRow(rows, slugColumn, wantedSlug, sheetName)', 'editor: row finder call')
text = replace_once(text, 'sheet: ENTITY_SHEET_NAME,', 'sheet: sheetName,', 'editor: report sheet')
text = replace_once(
    text,
    """    workbookPath,
    slug: args.slug,""",
...
if 'ENTITY_SHEET_NAME' in text:
    raise SystemExit('editor: unresolved ENTITY_SHEET_NAME reference')
