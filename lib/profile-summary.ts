 ...
 *
 * Everything written here is assembled from fields already on the record. No
 * claim is introduced that the record does not already make, and any clause
 * whose source field is missing is omitted rather than filled with a guess.
 */
...
function backingIsFalse(value: unknown): boolean {
...
export function buildProfileSummary(record: Record<string, unknown> | null | undefined): string {
  ...
  const authoredGrade = text(record.evidence_grade_source)
  const authoredGradeLabel = CANONICAL_GRADE_LABEL[authoredGrade as CanonicalEvidenceGrade]
  ...
  if (backingIsFalse(record.evidence_grade_backed) && authoredGradeLabel) {
    const [ratingPart] = authoredGradeLabel.split(/:\s*/)
    sentences.push(
      `${subject} has an authored ${ratingPart} rating, but the recorded studies do not currently demonstrate that strength.`,
    )
  } else if (gradeLabel) {
