...
export function buildAug23WorkbookEvidenceIndex(rawClaims: unknown): Map<string, Record<string, unknown>[]> {
  ...
  for (const value of rawClaims) {
    ...
    const current = byProfile.get(profileSlug)
...
function getAug23WorkbookEvidenceIndex() {
  if (!aug23WorkbookEvidenceIndexPromise) {
    aug23WorkbookEvidenceIndexPromise = readJsonFile('claims.json')
      .then(buildAug23WorkbookEvidenceIndex)
  }
