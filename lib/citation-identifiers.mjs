...
/**
 * The comparable identity of a citation title.
 *
 * The same-identifier-two-titles check exists to catch one PMID or DOI naming
 * two different studies. Comparing raw lowercased text made it fire on
 * typography instead: `Disorders-A Systematic Review` against
 * `Disorders—A Systematic Review`, and `... clinical trials.` against
 * `... clinical trials` are one paper cited by two profiles, not two papers.
 *
 * Dash style, quote style, trailing punctuation and repeated whitespace are
 * normalised away. Every word is kept, so two genuinely different studies
 * recorded under one identifier still conflict.
 *
 * @param {unknown} value
 * @returns {string}
 */
export function citationTitleIdentity(value) {
  return text(value)
    .toLowerCase()
    .replace(/[\u2010-\u2015\u2212]/g, '-')
    .replace(/[\u2018\u2019\u201b\u2032]/g, "'")
