...
const keys = ['herbs','compounds','goals','sleep','stress','anxiety','focus','methodology','safety'] as const
...
const englishFor = (key: string) => key === 'sleep' || key === 'stress' || key === 'anxiety' || key === 'focus' ? `/goals/${key}/` : key === 'methodology' ? '/info/methodology/' : key === 'safety' ? '/safety-checker/' : `/${key}/`
