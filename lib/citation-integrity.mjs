...
export function analyzeCitationIntegrity(profiles) {
  ...
  for (const profile of profiles) {
    ...
    for (const source of Array.isArray(record.sources) ? record.sources : []) {
      ...
      // A placeholder records that metadata is missing, not what the study is
      // called, so comparing one against a real title reports a conflict where
      // there is only an incomplete row - already counted in the advisories
      // above as `placeholder-title`.
      const titleIdentity = isPlaceholderCitationTitle(source.title) ? '' : citationTitleIdentity(source.title)
      for (const identifier of aliases) {
        const titles = seenByIdentifier.get(identifier) ?? new Set()
        if (titleIdentity) titles.add(titleIdentity)
