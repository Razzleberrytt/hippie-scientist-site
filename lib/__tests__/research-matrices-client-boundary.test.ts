...
describe('research matrix client boundary', () => {
  it('keeps the client component on the browser-safe shared contract', () => {
    const client = fs.readFileSync(
      path.join(process.cwd(), 'components/matrices/ResearchMatricesClient.tsx'),
      'utf8',
    )
  ...
  it('keeps filesystem-backed research topology and grade reconciliation on the server contract', () => {
    const shared = fs.readFileSync(
      path.join(process.cwd(), 'lib/research-matrices.shared.ts'),
      'utf8',
    )
    const server = fs.readFileSync(
      path.join(process.cwd(), 'lib/research-matrices.ts'),
      'utf8',
    )
    ...
    // src/lib/research-matrices.ts is now lib/research-matrices.ts, so the
    // fs-backed snapshot module is a sibling rather than two levels up.
    expect(server).toContain("from './research-quality-snapshot'")
