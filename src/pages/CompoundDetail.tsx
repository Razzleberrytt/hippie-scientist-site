...
export default function CompoundDetail() {
  ...
  const compoundClass = compound.compoundClasses?.[0] || compound.tags?.[0] || 'Unclassified'
  const researchRelevance =
    compound.description ||
    'This compound is tracked for potential psychoactive, neurochemical, or safety-relevant effects in botanical contexts.'
    ...
    <>
      ...
      <main className='container mx-auto max-w-3xl px-4 py-10 text-white'>
        <article className='space-y-6 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_30px_80px_rgba(0,0,0,.25)] backdrop-blur-xl'>
          ...
          {compound.description && <p className='text-white/80'>{compound.description}</p>}

          <section>
            <h2 className='text-sm font-semibold uppercase tracking-wide text-white/60'>
              Structure / Class
            </h2>
            <p className='mt-2 text-white/80'>{compoundClass}</p>
          </section>
          ...
          <section>
            ...
            <p className='mt-2 text-white/80'>{researchRelevance}</p>
          </section>
