...
import { Herb } from '../data/herbIndex';

interface UseHerbSearchProps {
  herbs: Herb[];
}

interface UseHerbSearchReturn {
  filteredHerbs: Herb[];
  searchTerm: string;
  ...
  setSelectedSafetyRange: (range: [number, number]) => void;
  sortBy: string;
...
export const useHerbSearch = ({ herbs }: UseHerbSearchProps): UseHerbSearchReturn => {
