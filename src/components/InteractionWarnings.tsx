...
export function InteractionWarnings({ edges, slugTypeMap }: InteractionWarningsProps) {
  ...
  const totalCount = bySeverity.reduce((sum, group) => sum + group.items.length, 0)
  const severitySummary = bySeverity
    .map(({ severity, items }) => `${items.length} ${SEVERITY_CONFIG[severity].label.toLowerCase()}`)
    .join(' · ')

  return (
    <details id='interactions' className='interaction-section-shell group/section scroll-mt-24 overflow-hidden'>
      <summary className='flex cursor-pointer items-center gap-3 p-5 sm:gap-4 sm:p-7'>
        <span className='editorial-icon-disc h-11 w-11 shrink-0' aria-hidden='true'>
          <ShieldAlert className='h-5 w-5 text-red-800 dark:text-red-200' strokeWidth={1.8} />
        </span>
        <div className='min-w-0 flex-1'>
          <p className='editorial-eyebrow'>Evidence-based safety</p>
          <h2 className='editorial-display mt-1 text-xl sm:text-2xl'>Caution when combined</h2>
          <p className='mt-1 text-xs font-semibold leading-5 text-[#647168] dark:text-[var(--text-muted)]'>
            {totalCount} flagged {totalCount === 1 ? 'pairing' : 'pairings'} · {severitySummary}
          </p>
        </div>
        <span aria-hidden='true' className='shrink-0'>
          <ChevronDown className='h-5 w-5 text-[#315f50] transition-transform group-open/section:rotate-180' strokeWidth={1.8} />
        </span>
      </summary>

      <div className='!mt-0 border-t border-[#123c2f]/10 px-5 pb-5 pt-4 sm:px-7 sm:pb-7'>
        <p className='max-w-3xl text-sm leading-7 text-[#526159] dark:text-[var(--text-secondary)] sm:text-base'>
            ...
            return (
              <details key={severity} className='interaction-severity-card group'>
                <summary className='flex cursor-pointer items-center gap-3 px-4 py-4 sm:px-5 sm:py-5'>
                  <span className='editorial-icon-disc h-11 w-11 shrink-0' aria-hidden='true'>
                    <ShieldAlert className={`h-5 w-5 ${config.accent}`} strokeWidth={1.8} />
                  </span>
                  <div className='min-w-0 flex-1'>
