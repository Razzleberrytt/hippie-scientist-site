    ...
    <div className='grid gap-8 lg:grid-cols-3'>
      <div className='space-y-6 lg:col-span-1'>
        <div className='space-y-4 rounded-3xl border border-brand-900/10 bg-white/90 p-5 shadow-sm'>
          <h2 className='text-lg font-bold text-ink'>Dose-math inputs</h2>
          ...
          <div className='space-y-1.5'>
            ...
            <select
              ...
              className='w-full rounded-2xl border border-brand-900/10 bg-[var(--surface-card)] px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none'
            >
              {allItems.map(item => (
          ...
          <div className='space-y-1.5'>
            <div className='flex items-center justify-between gap-3'>
              ...
              <span className='text-xs font-bold text-muted'>{extractPct}%</span>
            </div>
            <input
      ...
      <div className='space-y-6 lg:col-span-2'>
        <div className='space-y-6 rounded-[2rem] border border-brand-900/10 bg-white/90 p-6 shadow-sm sm:p-8'>
          <div className='border-b border-brand-900/10 pb-5'>
            <span className='rounded-full bg-[var(--surface-subtle)] px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-muted'>
              Study / monograph context only
            </span>
            <h2 className='mt-2 text-2xl font-bold text-ink sm:text-3xl'>{selectedItem.name} dose math</h2>
          ...
          <div className='rounded-2xl border border-brand-900/10 bg-[var(--surface-subtle)]/60 p-6'>
            <p className='text-[10px] font-bold uppercase tracking-wider text-slate-400'>Recorded source context</p>
            {selectedItem.dosage ? (
              <p className='mt-2 text-xl font-bold text-ink'>{selectedItem.dosage}</p>
            ) : (
              <p className='mt-2 text-sm font-semibold text-muted'>No dose range is recorded for this profile.</p>
            )}
          ...
          <div className='grid gap-6 sm:grid-cols-2'>
            <div className='space-y-2.5 rounded-2xl border border-brand-900/10 bg-[var(--surface-card)] p-5'>
              ...
              {parsedDose ? (
                ...
                <p className='text-sm font-semibold text-muted'>No safe automatic conversion available.</p>
              )}
            ...
            <div className='space-y-2.5 rounded-2xl border border-brand-900/10 bg-[var(--surface-card)] p-5'>
              ...
              {activeYield ? (
                ...
                <p className='text-sm font-semibold text-muted'>Set the label percentage to calculate.</p>
              )}
              <p className='text-xs leading-relaxed text-muted'>
          ...
          <div className='flex items-center justify-between gap-4 border-t border-brand-900/10 pt-3 text-xs'>
