...
export default function AuroraHero({ title, subtitle }: AuroraHeroProps) {
  ...
  return (
    ...
    >
      <motion.div aria-hidden className='aurora-bg absolute inset-0 -z-10' style={{ y: bgY }} />
      ...
      <div className='relative z-10 flex max-w-4xl flex-col items-center gap-6'>
        ...
        >
          <Link
            to='/herbs'
            className='btn-primary rounded-full px-6 py-2.5 text-sm transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] sm:text-base'
          >
            Browse Herbs
          </Link>
          <Link
            to='/blend'
            className='btn-secondary rounded-full px-6 py-2.5 text-sm transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] sm:text-base'
          >
            Build a Blend
          </Link>
        </motion.div>
