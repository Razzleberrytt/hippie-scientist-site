...
export default function LibraryBrowser({
  ...
  const filteredItems = useMemo(() => {
    ...
    return sortedItems.filter(item => {
      ...
      const matchesLetter = !letter || itemTitle.toUpperCase().startsWith(letter)
  ...
  return (
    <div className='mx-auto w-full max-w-7xl space-y-4 px-3 py-4 sm:px-6 lg:px-8'>
      <section className='overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.16),transparent_35%),linear-gradient(135deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))] p-4 shadow-xl shadow-black/20 sm:p-6'>
        <div className='flex items-start justify-between gap-3'>
          <div className='min-w-0'>
            <p className='text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-emerald-200/75'>{eyebrow}</p>
            <h1 className='mt-2 text-3xl font-bold tracking-tight text-white sm:text-5xl'>{title}</h1>
          </div>
      ...
      <section className='sticky top-0 z-20 -mx-3 border-y border-white/10 bg-[#080a12]/95 px-3 py-3 backdrop-blur sm:mx-0 sm:rounded-3xl sm:border sm:bg-white/[0.035] sm:p-3'>
        <label className='sr-only' htmlFor='library-search'>Search {title}</label>
        <input
          id='library-search'
          value={query}
        ...
        <div className='mt-3 flex gap-2 overflow-x-auto pb-2 [-webkit-overflow-scrolling:touch] scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/15'>
          <button
            type='button'
            onClick={() => setLetter('')}
            className={`min-h-10 shrink-0 rounded-full border px-4 py-2 text-xs font-bold transition ${letter === '' ? 'border-emerald-200 bg-emerald-300 text-slate-950 shadow-[0_0_18px_rgba(110,231,183,0.22)]' : 'border-white/10 bg-white/[0.04] text-white/72 hover:border-emerald-300/35 hover:bg-white/[0.08] hover:text-white'}`}
          >
            All
          </button>
          {LETTERS.map(item => (
            <button
              ...
              className={`min-h-10 min-w-10 shrink-0 rounded-full border px-3 py-2 text-xs font-bold transition ${letter === item ? 'border-emerald-200 bg-emerald-300 text-slate-950 shadow-[0_0_18px_rgba(110,231,183,0.22)]' : 'border-white/10 bg-white/[0.04] text-white/72 hover:border-emerald-300/35 hover:bg-white/[0.08] hover:text-white'}`}
            >
              {item}
            </button>
          ))}
      ...
      {filteredItems.length > 0 ? (
        <section className='grid gap-3 sm:grid-cols-2 xl:grid-cols-3'>{renderedCards}</section>
      ) : (
