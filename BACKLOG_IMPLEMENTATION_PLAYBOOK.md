...
This is the execution contract for agents working through `THS-0001` through `THS-1000`.

The system is designed for several specialist agents working in parallel without duplicating work, overwriting each other, or weakening evidence/safety standards.

Read this file **and** `backlog/AGENT_COORDINATION.md` before claiming work.

## Backlog files

- `backlog/master_backlog.csv.xz.b64` — immutable compressed/base64 definition seed for all 1,000 tickets.
- `backlog/status.csv` — mutable source of truth for status, owner, claim time, branch, blocker, PR/commit, and proof.
- `backlog/materialize_backlog.py` — generates the readable `backlog/master_backlog.csv` view.
- `backlog/AGENT_COORDINATION.md` — specialist lanes, claiming, ownership, collision prevention, stale claims, and coordinator rules.
...
Tickets already contain a specialty lane. Do not manually assign all 1,000 tickets one by one.

| Lane | Primary responsibility |
|---|---|
| Coordinator | Queue health, dependencies, claims, collisions, blocker triage, reprioritization |
...
| Evidence | Evidence grading, study quality, claim support, citations, methodology |
...
- **P2 Medium:** worthwhile optimization; order increasingly by real data.
...
- `Not Started` — defined but not eligible yet.
...
**Done never means “code was written.”** Done requires implementation, applicable tests/QA, merge/deploy where relevant, and proof.

## Claim before work

Before changing implementation files, claim the ticket in `backlog/status.csv`.

An active claim must contain:

- `ID`
- `Status=In Progress`
- `Owner`
- `Claimed At` as an ISO-8601 timestamp
- `Branch`
...
Publish the claim before substantive implementation. If another owner already has an active claim, do not start the ticket.
...
Do **not** execute tickets sequentially merely because their IDs are sequential.

## Standard specialist loop

1. Sync with the target branch.
2. Read this playbook and `backlog/AGENT_COORDINATION.md`.
3. Run `python backlog/materialize_backlog.py`.
4. Select the highest-priority eligible ticket in your lane.
5. Claim it in `backlog/status.csv` and publish the claim.
6. Inspect existing implementation, tests, data, content, and docs.
7. Implement only the ticket scope using the smallest durable solution.
8. Add/update appropriate regression coverage.
9. Run all applicable lint/type/build/test/visual/accessibility/data/SEO/E2E checks.
10. Move the ticket to `In Review` and record the PR.
11. Merge only after applicable acceptance and QA gates pass.
12. Mark `Done` in `status.csv`, recording final PR/commit and proof.
13. Sync again before claiming another ticket.
...
4. Detect overlapping foundational work and serialize it when necessary.
5. Allow each specialist to claim its highest-priority eligible ticket.
6. Resolve or route blockers and stale claims.
7. Reprioritize later work when Search Console, analytics, revenue, incidents, dependencies, or architecture changes materially alter ROI.
8. Keep concurrency intentionally limited until the claim/merge process is stable.

Recommended initial concurrency is about four implementation streams:
...
Growth can run concurrently when it does not depend on unfinished measurement or foundational UX work.

## One-owner rule

Every active ticket has exactly one implementation owner.

Other agents may review or supply specialty input, but they must not independently implement competing versions of the same ticket. For cross-specialty work, choose one owner and record supporting reviewers/work in `Proof / Notes`.
...
Prefer **one ticket / one PR** for high-risk work:
...
A micro-batch may share a PR only when splitting it would create a broken intermediate state. Record every included ticket ID.

## Definition of Ready

A ticket is `Ready` only when:

- dependencies are complete or verified unnecessary;
...
10. If a ticket could materially alter a health conclusion and the correct behavior is unclear, mark it `Blocked` with a concrete explanation.

## Design rules

- Establish/reuse tokens rather than multiplying one-off values.
- Reuse shared components before creating variants.
- Prefer one canonical pattern per content type.
- Test long titles, missing fields, dense warnings, and large evidence/reference sections.
...
Run python backlog/materialize_backlog.py to generate the current readable backlog.
...
1. Inspect existing implementation and relevant tests/data first.
2. Implement only the scoped requirement using the smallest durable solution.
3. Preserve existing working functionality and evidence/safety boundaries.
