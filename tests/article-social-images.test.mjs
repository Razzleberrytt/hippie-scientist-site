...
describe('article social preview cards', () => {
  ...
  it('keeps page metadata, Twitter metadata, and Article JSON-LD on one cache-keyed image authority', () => {
    ...
    const buildDeploy = fs.readFileSync('scripts/build-deploy.mjs', 'utf8')
    ...
    expect(buildDeploy).toContain("name: 'build-article-social-images'")
    expect(buildDeploy).toContain("cmd: 'node scripts/media/build-article-social-images.mjs'")
    expect(buildDeploy).toContain("'public/media/social/articles/**/*.jpg'")
    expect(buildDeploy).toContain("'public/media/social/articles/manifest.json'")
  })
})
