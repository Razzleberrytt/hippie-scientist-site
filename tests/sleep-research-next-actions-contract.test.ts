...
const sharedSource = fs.readFileSync(path.join(process.cwd(), 'components', 'ResearchNextActions.tsx'), 'utf8')
...
describe('SleepResearchNextActions contract', () => {
  ...
  it('preserves labelled section semantics and visible keyboard focus', () => {
    ...
    expect(sharedSource).toContain('aria-labelledby={headingId}')
    expect(sharedSource).toContain('id={headingId}')
    expect(sharedSource).toContain('focus-visible:ring-2')
  })

  it('preserves mobile-safe target sizing without sticky or fixed obstruction', () => {
    expect(sharedSource).toContain('min-h-11')
    expect(sharedSource).not.toMatch(/className="[^"]*\b(?:fixed|sticky)\b/)
  })
