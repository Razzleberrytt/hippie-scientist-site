...
export function DecisionEmptyState({
  ...
  return (
    <div className="rounded-[1.35rem] border border-brand-900/10 bg-white/85 p-5 shadow-[var(--shadow-card-calm)] sm:p-7">
      <div className="max-w-2xl space-y-2.5">
        <p className="eyebrow-label">{eyebrow}</p>
        <h2 className="compact-heading">{title}</h2>
        <p className="text-sm leading-6 text-[#46574d] sm:text-base">{description}</p>
        {currentScan ? (
          <p className="text-sm leading-6 text-[#5f6f66]">Current focus: {currentScan}</p>
        ) : null}
      </div>
    ...
    >
      <div className="flex flex-1 flex-col">
        <div className="flex items-start justify-between gap-3">
          ...
          {featured ? (
            <span className="shrink-0 rounded-full border border-brand-700/10 bg-brand-50 px-2.5 py-1 text-xs font-bold text-brand-800">Start here</span>
          ) : null}
        </div>
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          ...
          <DecisionMetric label="Timing" value={timeToEffect} />
        </div>
