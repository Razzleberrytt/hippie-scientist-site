...
export default function LocalizedCorePage({
  page,
  ui,
  lang,
}: {
    ...
    <div className='mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-16 lg:px-10' lang={lang}>
      ...
      {(page.primaryCta || page.secondaryCta) ? (
        <section className='mt-10 flex flex-col gap-3 rounded-[1.75rem] border border-[#123c2f]/10 bg-[#123c2f] p-6 text-[#fffdf8] sm:flex-row sm:items-center sm:justify-between sm:p-8'>
          <div>
            <p className='text-xs font-extrabold uppercase tracking-[0.14em] text-[#e2cba3]'>{ui.nextStepLabel}</p>
            <p className='mt-2 max-w-xl text-sm leading-6 text-[#f4efe5]'>{ui.nextStepBody}</p>
          </div>
