...
function LanguageLinks({ pathname, currentLocale, compact = false }: { pathname: string; currentLocale: SupportedLocale; compact?: boolean }) {
  ...
  return (
    <div role='group' className={`flex items-center ${compact ? 'gap-1' : 'gap-1.5'} overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden`} aria-label={currentConfig.languagesAriaLabel}>
      <Languages className='h-3.5 w-3.5 shrink-0 text-[var(--hs-gold)]' aria-hidden='true' />
      {links.map((link) => (
        <Link
          key={link.locale}
          href={link.href}
...
export default function LocalizedNavigation() {
  ...
  return (
    <nav
      className='site-primary-nav sticky top-0 z-[110] border-b border-[var(--border-soft)] bg-[color:var(--surface-card-strong)]/95 backdrop-blur-xl'
