...
export default function EvidenceMeterDetail({ data }: Props) {
  return (
    <div className="mt-3 space-y-3 text-sm">
      <p className="leading-6 text-[#46574d]">{data.explanation}</p>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span
          className="inline-flex items-center rounded-full border border-brand-900/10 bg-white/80 px-2.5 py-1 text-xs font-semibold text-[#5f6f66]"
      ...
      {data.downgradeReasons.length > 0 && (
        <div className="rounded-[0.65rem] border border-amber-200 bg-amber-50/70 p-3">
          <p className="text-[0.64rem] font-bold uppercase tracking-[0.09em] text-amber-800">
            Limitations
          </p>
          <ul className="mt-1.5 space-y-1">
            {data.downgradeReasons.map((reason) => (
              <li key={reason} className="text-xs leading-5 text-amber-900">
                {reason}
              </li>
            ))}
          </ul>
      ...
      <p className="text-[0.68rem] leading-5 text-[#7a8e82]">
        ...
        <Link
          ...
          className="font-semibold text-brand-700 underline underline-offset-2 hover:text-brand-800 focus:outline-none focus-visible:ring-1 focus-visible:ring-brand-700"
        >
