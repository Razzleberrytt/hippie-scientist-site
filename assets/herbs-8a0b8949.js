...
]`;function t(e){try{return JSON.parse(`"${e}"`)}catch{return e}}function o(e){const n=t(e),i=n.toLowerCase();return i==="ayahuasca-additive"?"Ayahuasca":i==="sleep"?"Dream":i==="sedative"?"Sedation":/(intense|strong|powerful)/.test(i)?"🔥 Intensity":/visionary|psychedelic|hallucinogenic/.test(i)?"Visionary":n}function h(e){const n=o(e).toLowerCase();return/(toxic|caution|restricted|safe)/.test(n)?"Safety":/(brew|tea|smoke|smokable|oral|chew|snuff|ferment|tincture|capsule|decoction|seed|root|bark|flower|incense)/.test(n)?"Preparation":/(stimulant|euphoria|dream|sleep|sedat|vision|dissoci|cognit|anxiolytic|energy|hallucinogen|calming|pain relief)/.test(n)?"Effect":/(dmt|lsa|thc|alkaloid|maoi|caffeine|nicotine|muscimol|morphine|tryptamine)/.test(n)?"Chemistry":/(africa|america|europe|asia|amazon|andes|australia|brazil|mexico|peru|india|china|pacific|region|north|south|west|east)/.test(n)?"Region":"Other"}function f(e){const n=o(e);return/toxic|restricted|caution/i.test(n)?"red":/safe/i.test(n)?"green":/stimulant|euphoria/i.test(n)?"pink":/dissoci|sedat/i.test(n)?"purple":/dream/i.test(n)?"blue":/cognitive/i.test(n)?"yellow":/brew|smok/i.test(n)?"blue":/oral|ferment/i.test(n)?"yellow":/ritual/i.test(n)?"green":"purple"}const d={"root bark":"root",bark:"root",tryptamine:"psychedelic",phenethylamine:"psychedelic",stimulants:"stimulant",stimulant:"stimulant",stim:"stimulant",stimulating:"stimulant",Stimulant:"stimulant",Stimulants:"stimulant",Stimulation:"stimulant","⚡ stimulant":"stimulant","☕ stimulant":"stimulant","⚠️ stimulating":"stimulant",relaxant:"sedative",relaxing:"sedative",relaxation:"sedative",calming:"sedative",calm:"sedative",sleep:"sedative",dream:"sedative","lucid dreaming":"sedative",hallucinogenic:"psychedelic",hallucinogen:"psychedelic",visionary:"psychedelic",entheogen:"psychedelic",dissociative:"psychedelic",euphoriant:"stimulant",energizing:"stimulant",energy:"stimulant",nootropic:"stimulant"};function l(e){const n=t(e).toLowerCase().trim();return d[n]||n}const u=/^(?:[\p{Emoji}\p{Extended_Pictographic}]|.*\b(?:tea|snuff|mood|flower)\b)/iu;function m(e){return Array.from(new Set(e.split(/[\/;,]/).map(n=>n.trim()).filter(Boolean).filter(n=>!/ritual/i.test(n)).map(n=>{const i=n.toLowerCase();return/(empathogen|euphoriant)/.test(i)?"Empathogen":/psychedelic|visionary/.test(i)?"Psychedelic":/dissociative|sedative/.test(i)?"Dissociative":/oneirogen|dream/.test(i)?"Oneirogen":/stimulant/.test(i)?"Stimulant":"Other"}))).slice(0,3)}const g=c.replace(/NaN/g,"null"),r=JSON.parse(g);r.forEach(e=>{if(Array.isArray(e.tags)){const n=e.tags.filter(a=>{const s=t(a).toLowerCase().trim();return!(/ritual/i.test(s)||u.test(s))}),i=new Map;n.forEach(a=>i.set(l(a),a)),e.tags=Array.from(i.values())}e.normalizedCategories=m(e.category),e.affiliateLink==null&&(e.affiliateLink="")});const p=r;export{r as a,h as b,l as c,t as d,d as e,p as h,o as n,f as t};
