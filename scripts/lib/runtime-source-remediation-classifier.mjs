...
function candidateIdentity(candidate) {
  return sourceIdentity({ doi: candidate?.doi, pmid: candidate?.pmid, canonicalUrl: candidate?.canonicalUrl })
}
...
function classifyOne({ record, orphanSourceId, localSources, candidates, reconciliationByCandidate }) {
  ...
  if (contradictoryCandidate) return { remediationClass: 'quarantine_unverifiable', reason: `Candidate ${contradictoryCandidate.candidateSourceId} shares an identity anchor but conflicts on another identifier.` }
  ...
  if (localSources.some(historicalSource)) return { remediationClass: 'historical_identity_recovery', reason: 'Local provenance explicitly marks this identity inactive, historical, deprecated, retired, or archived.' }
  ...
  return { remediationClass: 'identity_metadata_insufficient', reason: 'Profile-local metadata does not contain a safe DOI, PMID, or primary canonical identity anchor.' }
}
...
export function buildRuntimeSourceRemediationQueue({ orphanRows, entries, sourceCandidates = [], promotionReconciliations = [] }) {
  ...
  const queue = rows.map(row => {
    ...
    return { ...row, ...classification, published, safetyRelevant: signals.safetyRelevant, humanEvidenceRelevant: signals.humanEvidenceRelevant, sourceIdFanout: fanout, priorityScore, localSourceMetadataCount: localSources.length }
  }).sort((a, b) => b.priorityScore - a.priorityScore || String(a.sourceId).localeCompare(String(b.sourceId)) || String(a.kind).localeCompare(String(b.kind)) || String(a.slug).localeCompare(String(b.slug)))
  ...
  const sourceIdFanout = [...fanoutBySourceId.entries()].map(([sourceIdValue, fanout]) => ({ sourceId: sourceIdValue, fanout })).sort((a, b) => b.fanout - a.fanout || a.sourceId.localeCompare(b.sourceId))
  return {
    schemaVersion: 1,
    orphanRows: queue.length,
    uniqueAffectedProfiles: new Set(queue.map(item => `${item.kind}:${item.slug}`)).size,
    uniqueOrphanSourceIds: new Set(queue.map(item => item.sourceId)).size,
