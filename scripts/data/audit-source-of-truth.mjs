...
function hasAnyDataPattern(text) {
  return dataPatterns.find((pattern) => pattern.regex.test(text)) ?? null
}
...
function classifyFile(rel, text) {
  ...
  if (NEVER_BLOCK_PATHS.has(rel) || NEVER_BLOCK_PREFIXES.some((prefix) => rel.startsWith(prefix))) {
    return { classification: 'RUNTIME_OR_DOC_CODE', allowed: true }
  }
  if (rel === 'data/seoCollections.ts') {
    return { classification: 'RUNTIME_OR_DOC_CODE', allowed: true }
  }
  if (NON_BLOCKING_PREFIXES.some((prefix) => rel.startsWith(prefix))) {
  ...
  if (DOC_PREFIXES.some((prefix) => rel.startsWith(prefix))) {
    return { classification: 'STATIC_SITE_COPY', allowed: true }
  }
...
for (const abs of files) {
  ...
  if (!matched && !(DATA_CODE_PREFIXES.some((prefix) => rel.startsWith(prefix)) && looksLikeHardcodedDataBlob(text))) continue
...
const md = [
  ...
  '## Issues',
  '',
  ...issues.map((issue, index) => `### ${index + 1}. ${issue.severity.toUpperCase()} — ${issue.entityType}\n- path: \`${issue.path}\`\n- classification: ${issue.classification}\n- problem: ${issue.problem}\n- fixTarget: ${issue.fixTarget}\n- suggestedWorkbookAction: ${issue.suggestedWorkbookAction ?? 'n/a'}\n- suggestedProcessAction: ${issue.suggestedProcessAction ?? 'n/a'}\n`),
]
