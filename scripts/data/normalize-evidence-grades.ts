 ...
 * Authored values remain in *_source; public A/B strength fails closed when the
...
const PROFILE_SYNC_FIELDS = [
  ...
  'evidence_grade_backed', 'evidence_grade_backing_gap',
] as const
...
function syncDetailRecord(indexFile: string, row: Row): void {
  ...
  if (!slug) return
  const detailDir = indexFile === 'herbs.json' ? 'herbs-detail' : indexFile === 'compounds.json' ? 'compounds-detail' : null
  if (!detailDir) return
...
function normalizeProfiles(file: string, claimsBySlug: Map<string, Row[]>, sourcesBySlug: Map<string, Row[]>) {
  ...
  if (!rows.length) return { file, total: 0, adjusted: 0, rationaleCards: 0, summariesWritten: 0, reasons: {} as Record<string, number>, changes: [] as Row[], unbacked: [] as Row[] }
  ...
  const next = rows.map((row) => {
    ...
    const publicTier = backing.backed
      ? (publicBand ? BAND_LABEL[publicBand] : '')
...
function normalizeClaims() {
  ...
  if (!rows.length) return { total: 0, classes: {} as Record<string, number>, bySlug: new Map<string, Row[]>() }
  ...
  const next = rows.map((row) => {
    ...
    return { ...row, study_class: studyClass, study_class_source: String(row.study_class_source ?? row.evidence_tier ?? ''), study_class_label: info.label, study_class_rank: info.rank, study_class_human: info.human }
  })
...
function main() {
  ...
  console.log(`A/B source grades unbacked ${unbacked.length} (${unbackedIndexable.length} indexable; settled public grade withheld)`)
  const gapCounts: Record<string, number> = {}
  for (const row of unbackedIndexable) gapCounts[String(row.reason)] = (gapCounts[String(row.reason)] ?? 0) + 1
  for (const [reason, count] of Object.entries(gapCounts).sort((a, b) => b[1] - a[1])) console.log(`  ${String(count).padStart(5)}  ${reason}`)
  console.log(`\nClaims classified     ${totals.claims}`)
  for (const [studyClass, count] of Object.entries(claims.classes).sort((a, b) => b[1] - a[1])) console.log(`  ${String(count).padStart(5)}  ${studyClass}`)
