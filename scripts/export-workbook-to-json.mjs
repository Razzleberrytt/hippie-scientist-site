...
function exportHerbs(workbook, diagnostics, resolvedSheets) {
  ...
  const records = rows
    .map(row => ({
      ...
      publicationStatus: firstMeaningful(row.publicationStatus, row.publish_status, row.publishStatus),
      publish_status: firstMeaningful(row.publish_status, row.publicationStatus),
