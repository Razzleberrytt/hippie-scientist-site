...
function run() {
  ...
  const targetsPath = path.join(root, 'ops', 'reports', 'source-wave-1-targets.json')
  const candidatesPath = path.join(root, 'ops', 'reports', 'source-wave-1-candidates.json')
  const intakePath = path.join(root, 'ops', 'reports', 'source-intake-queue.json')
  const outPath = path.join(root, 'ops', 'reports', 'source-wave-1-summary.md')
  ...
  const lines: string[] = []
  lines.push('# Source Wave 1 Summary')
  lines.push('')
