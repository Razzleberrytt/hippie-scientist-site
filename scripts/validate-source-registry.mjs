...
import { UNIQUE_SOURCE_IDENTITY_FIELDS, sourceIdentityTokens } from './data/source-identity.mjs'
...
for (const [index, source] of registry.entries()) {
  ...
  for (const field of UNIQUE_SOURCE_IDENTITY_FIELDS) {
    for (const token of sourceIdentityTokens(field, source?.[field])) {
      const prior = identityOwners.get(token)
      if (prior && prior.sourceId !== sourceId) {
        issues.push(`${prefix} ${field} duplicates source identity ${token} owned by ${prior.sourceId}.`)
      } else if (!prior) {
