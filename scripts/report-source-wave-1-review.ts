...
const WAVE_TARGETS_PATH = path.join(ROOT, 'ops', 'reports', 'source-wave-1-targets.json')
const WAVE_CANDIDATES_PATH = path.join(ROOT, 'ops', 'reports', 'source-wave-1-candidates.json')
const OUTPUT_JSON = path.join(ROOT, 'ops', 'reports', 'source-wave-1-review.json')
const OUTPUT_MD = path.join(ROOT, 'ops', 'reports', 'source-wave-1-review.md')
...
function run() {
  ...
  for (const candidate of waveCandidates) {
    ...
    if (duplicateMatches.length > 0 || candidate.duplicateRisk === 'known-duplicate' || isNonEmpty(candidate.duplicateOfSourceId)) {
      ...
      promotedRows.push({
        ...
        reviewedAt: nowIso,
        notes: `Promoted via source-wave-1 review. intakeTaskId=${candidate.intakeTaskId}; candidateSourceId=${candidate.candidateSourceId}.`,
  ...
  const report = {
    generatedAt: nowIso,
    deterministicModelVersion: 'source-wave-1-review-v1',
    sources: {
  ...
  const md: string[] = []
  md.push('# Source Wave 1 Candidate Review and Registry Promotion')
  md.push('')
