...
const UNBOUND_EQUIVALENCE_LABEL_RE = /\b(?:equivalent\s+to|same\s+as|comparable(?:\s+(?:efficacy|effectiveness|effects?|benefits?))?\s+to|as\s+effective\s+as|equally\s+(?:effective|efficacious)\s+as|(?:more|less)\s+(?:effective|efficacious)\s+than|matches?\b[^.!?\n]{0,48}\b(?:efficacy|effectiveness|effects?|benefits?)|(?:superior|inferior|non[- ]?inferior)\s+to|outperform(?:s|ed|ing)?|works?\s+like)\b/i
