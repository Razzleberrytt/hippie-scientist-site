...
const UNBOUND_SAFETY_REGULATORY_LABEL_RE = /\b(?:fda[- ]?(?:approved|cleared)|approved\s+by\s+(?:the\s+)?fda|no\s+(?:known\s+)?(?:drug\s+)?interactions?|interaction[- ]?free|safe\s+(?:during|for|in)\s+(?:pregnancy|breastfeeding|children|kids|infants?|babies)|pregnancy[- ]?safe|breastfeeding[- ]?safe|child[- ]?safe|kid[- ]?safe|non[- ]?habit[- ]?forming|nonaddictive|not\s+addictive|addiction[- ]?free)\b/i
