...
const VISUAL_COMPLETE_ASSET_TYPES = new Set(['infographic', 'carousel'])
const CLAIM_CONTEXT_REQUIRED_ASSET_TYPES = new Set([...COMPLETE_ASSET_TYPES, ...VISUAL_COMPLETE_ASSET_TYPES])
...
const UNBOUND_DOSE_LABEL_RE = /\b\d+(?:\.\d+)?\s*(?:(?:million|billion|trillion)\s+)?(?:mg|mcg|(?:µ|μ|u)g|g|ml|i\.?\s*u\.?|cfu|capsules?|tablets?|drops?|scoops?)(?=\b|[\s.,;:!?()])/i
const UNBOUND_ONSET_LABEL_RE = /(?:\b(?:works?|acts?|kicks?\s+in|takes?\s+effect|onset|starts?\s+working|begins?\s+working|effects?\s+(?:begin|begins|start|starts))\b[^.!?\n]{0,48}\b(?:within|in|after)?\s*\d+(?:\.\d+)?[-\s]*(?:minutes?|mins?|hours?|hrs?|days?)\b|\b(?:takes?|needs?|requires?)\s+\d+(?:\.\d+)?[-\s]*(?:minutes?|mins?|hours?|hrs?|days?)\s+to\s+(?:work|act|take\s+effect)\b|\b\d+(?:\.\d+)?[-\s]*(?:minutes?|mins?|hours?|hrs?|days?)\b[^.!?\n]{0,24}\b(?:onset|effect)\b)/i
const UNBOUND_EQUIVALENCE_LABEL_RE = /\b(?:equivalent\s+to|same\s+as|comparable(?:\s+(?:efficacy|effectiveness|effects?|benefits?))?\s+to|as\s+effective\s+as|equally\s+(?:effective|efficacious)\s+as|(?:more|less)\s+(?:effective|efficacious)\s+than|(?:better|worse)\s+than|(?:greater|higher|lower|reduced)\s+(?:efficacy|effectiveness|effects?|benefits?)\s+than|matches?\b[^.!?\n]{0,48}\b(?:efficacy|effectiveness|effects?|benefits?)|(?:superior|inferior|non[- ]?inferior)(?:\s+(?:efficacy|effectiveness|effects?|benefits?))?\s+to|outperform(?:s|ed|ing)?|works?\s+like)\b/i
...
function allowedClaimMap(pack) { return new Map(pack.claims.map((claim) => [String(claim.id), clean(claim.publicSafeStatement)])) }
function allowedUncertaintyMap(pack) { return new Map(pack.uncertainties.map((item) => [String(item.id), clean(item.statement)])) }
function allowedSafetyMap(pack) { return new Map(pack.safety.map((item) => [String(item.id), clean(item.statement)])) }
...
export function validateFactualAssetCopy(packInput, asset) {
  ...
  const claims = allowedClaimMap(pack), uncertainties = allowedUncertaintyMap(pack), safetyStatements = allowedSafetyMap(pack)
  const seenClaims = new Set(), seenUncertainties = new Set(), seenSafety = new Set()
  for (const [index, line] of lines.entries()) {
    if (!line || typeof line !== 'object' || Array.isArray(line)) { errors.push(`line ${index + 1} must be an object`); continue }
    const role = clean(line.role), text = clean(line.text)
    if (!text) { errors.push(`line ${index + 1} text is required`); continue }
    ...
    if (role === 'claim') {
      const claimId = clean(line.claimId), allowed = claims.get(claimId)
      ...
      if (claim?.evidenceContext === 'preclinical' && HUMAN_BENEFIT_RE.test(text)) errors.push(`line ${index + 1} projects preclinical evidence into human benefit language`)
    } else if (role === 'uncertainty') {
      const uncertaintyId = clean(line.uncertaintyId), allowed = uncertainties.get(uncertaintyId)
      ...
      const safetyId = clean(line.safetyId), allowed = safetyStatements.get(safetyId)
  ...
  if (CLAIM_CONTEXT_REQUIRED_ASSET_TYPES.has(assetType)) {
    for (const claimId of claims.keys()) if (!seenClaims.has(claimId)) errors.push(`${COMPLETE_ASSET_TYPES.has(assetType) ? 'complete ' : ''}${assetType} is missing governed claim ${claimId}`)
    for (const uncertaintyId of uncertainties.keys()) if (!seenUncertainties.has(uncertaintyId)) errors.push(`${COMPLETE_ASSET_TYPES.has(assetType) ? 'complete ' : ''}${assetType} is missing governed uncertainty ${uncertaintyId}`)
  }
  if (SAFETY_REQUIRED_ASSET_TYPES.has(assetType)) for (const safetyId of safetyStatements.keys()) if (!seenSafety.has(safetyId)) errors.push(`${assetType} is missing governed safety statement ${safetyId}`)
