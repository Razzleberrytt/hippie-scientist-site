...
describe('distribution pack schema/validator parity', () => {
  ...
  it('keeps v1 no-rewrite, complete governed-safety, and no-consumer-instruction invariants explicit in both layers', () => {
    ...
    expect(validatorSource).toContain('must preserve all approved canonical safety claims')
    expect(validatorSource).toContain('must resolve exactly once to an approved has_safety_warning claim on the canonical source page')
    expect(validatorSource).toContain('must exactly equal approved canonical claim')
    expect(validatorSource).toContain('must contain the complete canonical safety set')
  })
})
