...
const OUTPUT_JSON = path.join(ROOT, 'ops', 'reports', 'enrichment-wave-1-authoring.json')
const OUTPUT_MD = path.join(ROOT, 'ops', 'reports', 'enrichment-wave-1-authoring.md')
...
function run() {
  ...
  const report = {
    ...
    deterministicModelVersion: 'enrichment-wave-1-authoring-v1',
    paths: {
  ...
  console.log(`[report-enrichment-wave-1-authoring] submissions=${waveSubmissions.length} promoted=${promotion.promoted}`)
}
