...
export function reconcilePersistentGovernorState({ now = Date.now(), write = true } = {}) {
  return withWriterLock(() => {
    ...
    if (write && changed) {
      ...
      appendJsonl(statePath('ledger.jsonl'), {
        event: 'governor_state_reconciled',
        at,
        ...reconciled.metrics,
      })
    }
    ...
    return {
      ...
      activeLeases: (reconciled.queue.leases || []).length,
      queued: (reconciled.queue.queued || []).length,
      blocked: (reconciled.queue.blocked || []).length,
    }
  })
}
