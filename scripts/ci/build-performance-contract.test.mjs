...
describe('CI build performance contracts', () => {
  ...
  it('bounds the measured midpoint static-page concurrency trial to GitHub Actions only', () => {
    const config = read('next.config.mjs')

    expect(config).toContain("staticGenerationMaxConcurrency = process.env.GITHUB_ACTIONS === 'true' ? 14 : 8")
