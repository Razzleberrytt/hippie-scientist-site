...
const byTitle = new Map(), byDesc = new Map(), byCanonical = new Map()\nconst missingTitles = [], missingDescriptions = [], missingCanonicals = []
for (const r of routes) {
  ...
  if (canonical) byCanonical.set(canonical, [...(byCanonical.get(canonical)||[]), routePath]); else missingCanonicals.push(routePath)
}
const dup = (m)=>[...m.entries()].filter(([,v])=>v.length>1).map(([k,v])=>({value:k,routes:v}))
const report={generatedAt:new Date().toISOString(), duplicateTitles:dup(byTitle), duplicateDescriptions:dup(byDesc), duplicateCanonicals:dup(byCanonical), missingTitles, missingDescriptions, missingCanonicals}
...
if (report.duplicateCanonicals.length || report.duplicateTitles.length > 0 || report.duplicateDescriptions.length > 0 || missingTitles.length || missingDescriptions.length || missingCanonicals.length) { console.error('[metadata-audit] severe collisions found: unique descriptions assert failed'); process.exit(1)}
