...
const entries = [
  {
    ...
    record: {
      ...
      sources: [{ id: 'src_recoverable', doi: '10.1002/example.1', title: 'Randomized clinical trial' }],
      claimMap: [{ id: 'c1', claim: 'Human randomized trial with adverse event monitoring', evidenceLevel: 'human-clinical', sourceRefIds: ['src_recoverable'] }],
    },
  },
  {
    kind: 'herb',
    record: {
      slug: 'historical',
      governance: { indexingAllowed: false },
      sources: [{ id: 'src_historical', pmid: '12345678', active: false, title: 'Archived clinical record' }],
      claimMap: [{ id: 'c2', sourceRefIds: ['src_historical'] }],
    },
  },
  {
    kind: 'compound',
    record: {
      slug: 'candidate-conflict',
      governance: { indexingAllowed: true },
      sources: [{ sourceId: 'src_candidate', doi: '10.1002/example.2', pmid: '23456789', title: 'Systematic review' }],
      claimMap: [{ id: 'c3', claim: 'Systematic review in adults', evidenceLevel: 'human-clinical', sourceRefIds: ['src_candidate'] }],
    },
  },
  {
    kind: 'compound',
    record: {
      slug: 'insufficient',
      governance: { indexingAllowed: false },
      sources: [{ id: 'src_insufficient', title: 'Local citation without canonical identity' }],
      claimMap: [{ id: 'c4', sourceRefIds: ['src_insufficient'] }],
    },
  },
  {
    kind: 'herb',
    record: {
      ...
      governance: { indexingAllowed: true },
      sources: [
        { id: 'src_quarantine', doi: '10.1002/example.3', title: 'First identity' },
        { id: 'src_quarantine', doi: '10.1002/example.4', title: 'Conflicting identity' },
      ],
      claimMap: [{ id: 'c5', claim: 'Safety toxicity signal', sourceRefIds: ['src_quarantine'] }],
    },
  },
  {
    kind: 'herb',
    record: {
      ...
      governance: { indexingAllowed: false },
      sources: [{ id: 'src_recoverable', doi: '10.1002/example.1', title: 'Randomized clinical trial' }],
      claimMap: [{ id: 'c6', sourceRefIds: ['src_recoverable'] }],
    },
  },
]
...
const sourceCandidates = [
  {
    candidateSourceId: 'cand_candidate',
    doi: '10.1002/example.2',
    pmid: '23456789',
    reviewStatus: 'approved_for_registry',
    approvalNotes: 'Promoted to source registry during prior workflow.',
  },
]

const result = buildRuntimeSourceRemediationQueue({
  orphanRows,
  entries,
  sourceCandidates,
  promotionReconciliations: [],
})
...
assert.equal(result.countsByClass.recoverable_verified_identity, 2)
...
assert.equal(bySlug.get('recoverable').remediationClass, 'recoverable_verified_identity')
...
const missingLocal = buildRuntimeSourceRemediationQueue({
  orphanRows: [{ kind: 'herb', slug: 'missing-local', sourceId: 'src_missing', url: '/herbs/missing-local/' }],
  entries: [{ kind: 'herb', record: { slug: 'missing-local', sources: [], claimMap: [] } }],
})
assert.equal(missingLocal.queue[0].remediationClass, 'identity_metadata_insufficient')

const malformed = buildRuntimeSourceRemediationQueue({
  orphanRows: [{ kind: 'herb', slug: 'malformed', sourceId: 'src_bad', url: '/herbs/malformed/' }],
  entries: [{ kind: 'herb', record: { slug: 'malformed', sources: [{ id: 'src_bad', doi: 'not-a-doi' }], claimMap: [] } }],
})
assert.equal(malformed.queue[0].remediationClass, 'quarantine_unverifiable')

assert.throws(() => buildRuntimeSourceRemediationQueue({
  orphanRows: [orphanRows[0], orphanRows[0]],
  entries,
}), /Duplicate runtime registry orphan rows/u)
