...
export async function cancelRunWithFallback({ repo, runId, request = github, classifyOptions }) {
  ...
  if (!normal.conflict) return { cancelled: true, mode: 'cancel' }
  ...
  if (!liveDecision.cancel) {
    return { cancelled: false, mode: null, liveRun, liveDecision, reason: 'became-ineligible-before-force-cancel' }
  }

  const forced = await request(`/repos/${repo}/actions/runs/${runId}/force-cancel`, { method: 'POST' })
  if (!forced.conflict) return { cancelled: true, mode: 'force-cancel', liveRun, liveDecision }

  return { cancelled: false, mode: 'force-cancel', liveRun, liveDecision, reason: 'force-cancel-conflict' }
}
...
async function main() {
  ...
  const classified = selectRunsForCancellation([...deduped.values()], { nowMs, thresholdMs, activePrHeadShas, currentRunId })

  const receipt = {
    generatedAt: new Date(nowMs).toISOString(), repository: repo, dryRun, thresholdHours,
    currentRunId: currentRunId || null, selected: [], retained: [], cancelled: [], raced: [],
  }
  ...
  if (!dryRun && receipt.selected.length > 0) {
    ...
    for (const selected of receipt.selected) {
      ...
      const classifyOptions = { nowMs: Date.now(), thresholdMs, activePrHeadShas: freshActivePrHeadShas, currentRunId }
      const liveDecision = classifyRun(liveRun, classifyOptions)
      if (!liveDecision.cancel) {
        receipt.raced.push(receiptRow(liveRun, liveDecision, thresholdMs))
        continue
      }
      const result = await cancelRunWithFallback({ repo, runId: liveRun.id, classifyOptions })
      const finalRun = result.liveRun || liveRun
      const finalDecision = result.liveDecision || liveDecision
      if (result.cancelled) {
        receipt.cancelled.push({ ...receiptRow(finalRun, finalDecision, thresholdMs), cancellationMode: result.mode })
      } else {
        receipt.raced.push({
