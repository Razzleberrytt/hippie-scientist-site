...
| Build failures | Main-branch production/deploy failures | Current main (`0f70ea1`) passes validation/tests/data and the production build application but remains release-blocked. PR #4950 exact-head output evidence clears the prior orphan gate (`blockingOrphan=0`), then fails sitemap completeness because `/evidence` and `/info` are absent from every advertised sitemap. Production deploy is therefore unverified. | GitHub Actions current-main and PR #4950 exact-head run evidence | 2026-08-31 | Engineering lead | Resolve the separately evidenced sitemap completeness regression, rerun PR #4950 exact-head release checks, then merge and verify post-merge main before claiming recovery | A successful build application or isolated internal-link audit is not a successful output/deploy gate; no production status is inferred |
...
| Orphan pages | Canonical pages with no qualifying internal links under audit definition | PR #4950 exact-head output audit reports 32 total, 0 crawlable blockers, and 32 non-blocking/noindex cases after adding the comparison-hub link; current main still has the prior 33/1 snapshot until merge | PR #4950 exact-head build output/internal-link audit | 2026-08-31 | Content SEO | Keep #4949 scoped and unmerged until the required release suite is green, then verify the same result on post-merge main | This proves the route is discoverable under the repository audit definition; it does not prove traffic, ranking, indexing, or production deployment |
