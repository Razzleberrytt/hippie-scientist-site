...
| Build failures | Main-branch production/deploy failures | Current main (`0f70ea1`) passes validation/tests/data and the production build application, but output/SEO verification fails on one blocking orphaned crawlable route: `/guides/compare/caffeine-vs-caffeine-plus-l-theanine/`. Production deploy is therefore unverified for this revision. | GitHub Actions current-main run evidence | 2026-08-31 | Engineering lead | Complete #4949 and verify exact-head output/SEO plus post-merge main before claiming release recovery | A successful build application is not a successful output/deploy gate; the earlier historical schema/performance diagnostics remain separate unless independently retested |
...
| Orphan pages | Canonical pages with no qualifying internal links under audit definition | 33 total under current-main output audit; 1 crawlable blocker (`/guides/compare/caffeine-vs-caffeine-plus-l-theanine/`) and 32 non-blocking/noindex cases | Current-main build output/internal-link audit | 2026-08-31 | Content SEO | Complete #4949 by linking the built page from the existing comparison hub, then rerun full output/SEO verification | The blocking route was added without a hub inventory entry; no traffic or ranking outcome is implied |
