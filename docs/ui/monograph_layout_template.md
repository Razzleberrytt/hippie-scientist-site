...
## 0. Route, Data & Deployment Guardrails

- Keep `/herbs/:slug` and `/compounds/:slug` stable. If a slug must change, add a
  redirect in `public/_redirects`.
- Source broad structured content from `data-sources/herb_monograph_master.xlsx`,
  then run `npm run data:build`.
- Validate `slug`, display name, summary, evidence tier, and safety fields before
  writing JSON artifacts.
- Do not add API routes, middleware, server actions, runtime revalidation, or
  `next/headers`; the deployment is static export.
- Keep first render lean: hero, quick facts, safety, and primary links should not
  require loading large optional detail blocks.
...
- **Spacing:** Generous padding (`py-8 px-4 sm:px-6 lg:px-8`) to prevent dense text blocks.
- **Theme parity:** Light and dark mode must both preserve contrast for badges,
  callouts, links, and affiliate disclosure text.
...
- **Fallback:** If botanical name is missing, omit the subheading rather than
...
- **Disclosure:** Render `AffiliateDisclosure` before the first affiliate link.
- **Config:** Use `AFFILIATE_TAGS.amazon`; never hardcode a tag.
...
- **Schema:** Keep entity schema, `BreadcrumbList`, and FAQ schema deduplicated.
- **Evidence:** Label human, animal, in vitro, and traditional-use support distinctly.
- **Safety:** Show safety content even when efficacy content is thin.
- **Verification:** For route changes, run `npm run build && npm run verify:output`.
  For template-only doc updates, run `npm run typecheck`.
