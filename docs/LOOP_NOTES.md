...
## 2026-07-13 (later) — Did that recommended detail-file backfill re-commit; found the drift is wider than compounds-detail

Ran `npm run data:build` once from a clean tree (no workbook edits) exactly as
prescribed above. Confirmed the drift surface is bigger than previously
scoped: besides `herbs.json`/`compounds.json` (guarded by
`guard-no-full-build-drift.mjs`) and `compounds-detail`/`herbs-detail`, a
plain full build also non-deterministically reorders relationship arrays in
`docs/internal-link-map.md` (`Related Guides`/`Clusters` list order) and
bumps the timestamp in `ops/audit/governance-overlay-report.json` — neither
is part of `data:build:core`, both are pure noise, and I reverted both via
`git checkout --`. Worth adding to the guard's mental model even though
they're outside its two guarded files: **any file whose header says
"Generated by `npm run data:build`"** is a full-build-only artifact and
should never be committed from a bare full-build run without diffing it
for actual content first.

Kept 23 of the 28 detail-file diffs the build produced, all identical in
shape: a stale `safety-context-missing` indexability_reason flipping to
`safety-context` plus the matching `indexability_score` bump (+5 to +15),
for compounds whose real safety prose was already sourced into
`compounds.json` in earlier cycles but whose detail record had never been
recomputed. Reverted the other 5 (`caffeine-l-theanine`, `fadogia-agrestis`,
`peppermint-oil`, `saccharomyces-boulardii`, plus `magnesium` and
`melatonin`) plus `ashwagandha`/`maca`/`rhodiola` herbs-detail after
confirming each diff was pure `sources[]` reorder with zero
indexability/safety signal (checked programmatically: grepped each diff for
added/removed `indexability_score`/`indexability_reasons`/`safety-context`
lines and manually eyeballed the remainder for anything besides `src_*`
citation-object lines and bare `{`/`}` brace churn). `magnesium` and
`melatonin` were notable because they'd already been fixed in a **prior**
cycle (see the `90ff8fc` entry above) — this run's diff for them was 100%
reorder noise, confirming the backfill script is idempotent and doesn't
need re-running for already-fixed entries.

After reverting the noise, `npm run data:build:core` regenerated
`herbs.json`/`compounds.json`/`summary-indexes`/`search-index` byte-for-byte
identical to the pre-full-build committed versions (confirmed via
`git status` showing zero diff there), which is exactly what
`guard-no-full-build-drift.mjs` expects and a good sanity check that the
workbook itself hadn't changed. `guard:source-of-truth`, `data:validate`,
`npm run check`, and the full Vitest suite (578 tests) all pass clean on
the resulting 23-file diff.

Remaining thread for a future cycle: this only closes the
already-discovered detail-file drift, not the underlying 83-of-88
severity-tier-only `contraindications_or_flags` compounds still needing
real sourced prose (tracked since the `b6859c7`/multi-entry thread above).
