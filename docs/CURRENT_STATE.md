...
**Verified:** The repository does not currently demonstrate a complete measured growth loop. Search, engagement, affiliate-click, revenue, and email performance exports were unavailable. Historical 2026-08-21 validation found schema/media governance, site-health, and Lighthouse failures, but that old workflow status is no longer current: on 2026-08-25 the latest inspected main commit `39455dd` completed CI, Site Health Check, Production Content Lint, and Lighthouse successfully, and a later main revision also completed the Cloudflare Pages deployment workflow successfully. Full schema policy's 38 identity findings and historical Lighthouse performance diagnostics remain separate remediation evidence unless independently retested and closed. Evidence-governance rules also demote most generated profiles from indexing; sampled flagship ashwagandha and L-theanine profiles were live but `noindex, follow`.
...
**Verified limitations:** most profiles are withheld from indexing; historical schema/performance diagnostics remain open until independently retested or closed even though the latest inspected CI/Lighthouse workflow state is green; legacy/pre-build publication artifacts are not authoritative for final eligibility; commercial measurement is unverified; most configured product links are generic searches; and duplicate/thin intent surfaces remain.
...
**Verified:** At commit `24e50c56c` on 2026-08-21, Deploy and core CI workflows succeeded, while Schema and Media Governance, Site Health Check, and Lighthouse failed. During that reset, a fresh local production build and full output verification passed, and the strict cluster-member trust audit cleared the site-health runtime mismatch. That historical snapshot is superseded for workflow health by 2026-08-25 evidence: latest inspected main `39455dd` completed CI, Site Health Check, Production Content Lint, and Lighthouse successfully, while a later main revision completed Cloudflare Pages deploy successfully. Full schema policy's 38 first-party identity findings and the earlier Lighthouse performance diagnostics remain quality debt unless separately retested and closed; current workflow success does not erase those diagnostics.
...
| Historical quality diagnostics remain open despite current workflow recovery | The 2026-08-21 local/schema snapshot reported 38 blocking identity findings and Lighthouse performance failures; the latest inspected main Lighthouse workflow (#5708 on `39455dd`) now succeeds, so those diagnostics are remediation evidence rather than an active Lighthouse workflow blocker | Structured-data inconsistency or slower UX may remain even though current automation is green | High for historical findings; current diagnostic status requires targeted retest | Users, indexing, agent execution | Execute SEO-003 and PERF-001 as scoped diagnostic work; do not reopen the resolved repo-wide deploy blocker without new failing evidence |
...
| Historical Lighthouse performance diagnostics need explicit trend verification | The 2026-08-21 Lighthouse run reported CLS, LCP, and TBT assertion failures while accessibility passed; latest current-main Lighthouse run #5708 succeeds, so the old result no longer describes workflow health | Performance may still affect search/user experience and conversion if the underlying metrics remain elevated | Medium until current metrics are compared directly | Users, revenue | Re-run/inspect PERF-001 metric evidence on current templates and close or refresh the historical findings explicitly |
